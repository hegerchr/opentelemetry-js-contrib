...
export class TraceExportEnabledValidator extends AbstractSourcePolicyValidator {
  getPolicyType(): string {
    return 'TraceExportEnabledPolicy';
  }

  validateJsonValue(value: unknown): TelemetryPolicy | null {
    if (typeof value !== 'boolean') return null;
    return new TraceExportEnabledPolicy(value);
  }

  validateKeyValueValue(value: string): TelemetryPolicy | null {
    if (value === 'true') return new TraceExportEnabledPolicy(true);
    if (value === 'false') return new TraceExportEnabledPolicy(false);
    return null;
  }
}
