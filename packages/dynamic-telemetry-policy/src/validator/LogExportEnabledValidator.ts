...
export class LogExportEnabledValidator extends AbstractSourcePolicyValidator {
  getPolicyType(): string {
    return 'LogExportEnabledPolicy';
  }

  validateJsonValue(value: unknown): TelemetryPolicy | null {
    if (typeof value !== 'boolean') return null;
    return new LogExportEnabledPolicy(value);
  }

  validateKeyValueValue(value: string): TelemetryPolicy | null {
    if (value === 'true') return new LogExportEnabledPolicy(true);
    if (value === 'false') return new LogExportEnabledPolicy(false);
    return null;
  }
}
